...
function apply(ctx) {
  ...
  function ensureDay(date) {
    ...
    if (day === undefined) {
      day = { turns: 0, tokens: { input: 0, output: 0, cacheRead: 0, cacheWrite: 0, reasoning: 0 }, perWs: new Map(), byWs: new Map() }
      byDay.set(date, day)
    }
    return day
  }
  ...
  function addUsage(wsId, time, usage) {
  ...
  function foldEvent(wsId, time, type, data) {
    ...
    else if (type === 'assistant/message' && data && data.usage) addUsage(wsId, time, data.usage)
  }
  ...
  async function fetchBalance(force) {
    ...
    const attempts = [
      // Linux/macOS: plain `curl` (curl.exe is Windows-only and fails to spawn here)
